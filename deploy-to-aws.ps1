...
# This script helps you deploy Laravel CI/CD to AWS production
...
function Setup-AWS {
    ...
    Write-Host "🏗️ Creating IAM user for GitHub Actions..." -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host "⚠️ IAM user may already exist" -ForegroundColor Yellow
    }
    ...
    foreach ($policy in $policies) {
        try {
            ...
            Write-Host "⚠️ Failed to attach: $($policy.Split('/')[-1])" -ForegroundColor Yellow
        }
    }
...
function Check-Status {
    ...
    try {
        ...
        if ($cluster.clusters.Count -gt 0) {
            ...
            Write-Host "⚠️ ECS Cluster not found" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "⚠️ Cannot check ECS cluster" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        if ($service.services.Count -gt 0) {
            ...
            Write-Host "⚠️ ECS Service not found" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "⚠️ Cannot check ECS service" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Host "⚠️ Cannot check ECR repository" -ForegroundColor Yellow
    }
    
    # Check load balancer
    try {
        $lbs = aws elbv2 describe-load-balancers 2>$null | ConvertFrom-Json
        $laravelLB = $lbs.LoadBalancers | Where-Object { $_.LoadBalancerName -like "*laravel*" }
        if ($laravelLB) {
            Write-Host "✅ Load Balancer found" -ForegroundColor Green
            Write-Host "   DNS: $($laravelLB.DNSName)"
            Write-Host "   State: $($laravelLB.State.Code)"
        } else {
            Write-Host "⚠️ Load Balancer not found" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "⚠️ Cannot check Load Balancer" -ForegroundColor Yellow
    }
